...
namespace dumper::stages::special_mesh {
    ...
    auto dump() -> bool {
        ...
        if (mesh_offset) {
            g_dumper.add_offset("SpecialMesh", "Offset", *mesh_offset, "", FieldType::Vector3);
        } else {
        ...
        if (scale_offset) {
            g_dumper.add_offset("SpecialMesh", "Scale", *scale_offset, "", FieldType::Vector3);
        } else {
