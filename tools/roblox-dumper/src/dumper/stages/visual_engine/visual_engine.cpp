...
namespace dumper::stages::visual_engine {
    ...
    static auto dump_render_view(const uintptr_t visual_engine) -> bool {
        ...
        g_dumper.add_offset("RenderView", "SkyboxValid", 0x28d, "",
                            FieldType::UInt16); // hardcoded for now

        const auto device_offset = process::Rtti::find(*render_view, "DeviceD3D11@Graphics@RBX");

        if (device_offset) {
            g_dumper.add_offset("RenderView", "DeviceD3D11", *device_offset, "",
                                FieldType::Pointer);
        }
    ...
    static auto dump_view_matrix(const uintptr_t visual_engine) -> bool {
        ...
        for (size_t offset = 0; offset < 0x2000; offset += 0x10) {
            ...
            if (is_valid_view_matrix(mat)) {
                g_dumper.add_offset("VisualEngine", "ViewMatrix", offset, "", FieldType::Matrix4x4);
                return true;
            }
        }
