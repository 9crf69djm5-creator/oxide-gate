...
namespace dumper::stages::player {
    auto dump() -> bool {
        ...
        dumper::g_dumper.add_offset("Player", "Character", *character, "", FieldType::Pointer);

        const auto local_player_char =
            process::Memory::read<roblox::Instance>(*local_player_addr + *character);

        if (!local_player_char) {
            spdlog::error("Failed to find LocalPlayer character");
            return false;
        }

        dumper::g_local_player_char = *local_player_char;
        ...
        FIND_AND_ADD_OFFSET(*local_player_addr, Player, uint32_t, AccountAge,
                            player_info->account_age, 0x600, 0x4, FieldType::UInt32);
        ...
        std::optional<size_t> team_ptr;
        for (size_t off = 0x100; off < 0x400; off += 0x8) {
            auto ptr = process::Memory::read<uintptr_t>(*local_player_addr + off);
            if (!ptr || *ptr < 0x10000 || *ptr > 0x7FFFFFFFFFFF)
                continue;
            roblox::Instance inst(*ptr);
            auto cn = inst.get_class_name();
            if (cn && *cn == "Team") {
                team_ptr = off;
                break;
            }
        ...
        dumper::g_dumper.add_offset("Player", "Team", *team_ptr, "", FieldType::Pointer);
