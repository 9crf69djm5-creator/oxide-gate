...
namespace dumper::stages::atmosphere {
    auto dump() -> bool {
        ...
        FIND_AND_ADD_OFFSET(atmosphere->get_address(), Atmosphere, float, Glare, 0.432f, 0x400, 0x4,
                            FieldType::Float);
        ...
        if (color_offset) {
            g_dumper.add_offset("Atmosphere", "Color", *color_offset, "", FieldType::Color3Float);
        } else {
        ...
        if (decay_offset) {
            g_dumper.add_offset("Atmosphere", "Decay", *decay_offset, "", FieldType::Color3Float);
        } else {
