...
namespace dumper::stages::lighting {
    auto dump() -> bool {
        ...
        dumper::g_dumper.add_offset("Lighting", "Sky", *sky, "", FieldType::Pointer);
        ...
        dumper::g_dumper.add_offset("Lighting", "Atmosphere", *atmosphere, "", FieldType::Pointer);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, float, Brightness, 3.567f,
                            0x400, 0x4, FieldType::Float);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, uint64_t, ClockTime,
                            32400000000, 0x400, 0x8, FieldType::UInt64);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, float,
                            EnvironmentDiffuseScale, 0.678, 0x400, 0x4, FieldType::Float);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, float,
                            EnvironmentSpecularScale, 0.762, 0x400, 0x4, FieldType::Float);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), LightingParameters, float,
                            GeographicLatitude, 115.9, 0x400, 0x4, FieldType::Float);

        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, float,
                            ExposureCompensation, -1.572f, 0x400, 0x4, FieldType::Float);
        ...
        FIND_AND_ADD_OFFSET(dumper::g_lighting->get_address(), Lighting, float, ShadowSoftness,
                            0.456f, 0x400, 0x4, FieldType::Float);
        ...
        if (ambient_offset) {
            g_dumper.add_offset("Lighting", "Ambient", *ambient_offset, "", FieldType::Color3Float);
        } else {
        ...
        g_dumper.add_offset("LightingParameters", "Source", *light_direction_offset + 0xC, "",
                            FieldType::UInt8);
        g_dumper.add_offset("LightingParameters", "TrueSunPosition", *light_direction_offset + 0x10,
                            "", FieldType::Vector3); // same as light direction
        ...
        dumper::g_dumper.set_namespace_comment("LightingParameters",
