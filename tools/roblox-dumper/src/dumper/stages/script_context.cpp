    ...
    auto dump() -> bool {
        ...
        const auto require_bypass = process::g_xref.instruction_scan(
            ...
            [](const ZydisDecodedInstruction& instr, const ZydisDecodedOperand* operands) {
                return process::helpers::zydis::is_cmp_mem_zero(instr, operands, ZYDIS_REGISTER_RDX,
                                                                0x100);
            });
        ...
        const auto is_roblox_script = process::g_xref.instruction_scan(
            search_start, buffer,
            [](const ZydisDecodedInstruction& instr, const ZydisDecodedOperand* operands) {
                return process::helpers::zydis::is_test_mem_imm(instr, operands, ZYDIS_REGISTER_RAX,
                                                                0x100);
            });
        ...
        const auto is_roblox_script_offset = is_roblox_script->operands[0].mem.disp.value;
