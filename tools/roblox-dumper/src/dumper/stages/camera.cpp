...
namespace dumper::stages::camera {
    ...
    auto dump() -> bool {
        ...
        FIND_AND_ADD_OFFSET(camera->get_address(), Camera, float, FieldOfView, fov_radians, 0x400,
                            0x4, FieldType::Float);

        glm::vec3 camera_position(45.2f, 19.4f, 50.0f);
        const auto position_offset = process::helpers::find_vec_offset(
            camera->get_address(), camera_position, 0x400, 5.0f, 0x4);
        ...
        g_dumper.add_offset("Camera", "CFrame", *position_offset - 0x24, "", FieldType::CFrame);
        ...
        const auto viewport_offset = process::helpers::find_vec_offset(
            camera->get_address(), viewport_size, 0x400, 5.0f, 0x4);
        ...
        g_dumper.add_offset("Camera", "ViewportSize", *viewport_offset, "", FieldType::Vector2);
        ...
        if (dumper::g_local_player_char && dumper::g_local_player_char->is_valid()) {
            auto humanoid = dumper::g_local_player_char->find_first_child_of_class("Humanoid");
            if (humanoid && humanoid->is_valid()) {
                auto subject_offset = process::helpers::find_pointer_offset(
                    camera->get_address(), humanoid->get_address(), 0x300);
                if (subject_offset)
                    g_dumper.add_offset("Camera", "CameraSubject", *subject_offset, "",
                                        FieldType::Pointer);
                else
                    spdlog::error("Camera: Failed to find CameraSubject offset");
            }
        }
