...
namespace process {
    ...
    auto Process::get_window_dimensions() const -> std::optional<glm::vec2> {
        ...
        if (width <= 0 || height <= 0) {
            ...
            RECT chrome{0, 0, 0, 0};
            AdjustWindowRectEx(&chrome, style, FALSE, ex_style);

            int window_w = r.right - r.left;
            int window_h = r.bottom - r.top;
            int chrome_w = (chrome.right - chrome.left);
            int chrome_h = (chrome.bottom - chrome.top);

            width = static_cast<float>(window_w - chrome_w);
            height = static_cast<float>(window_h - chrome_h);
